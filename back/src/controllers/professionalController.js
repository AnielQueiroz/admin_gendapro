...
const createProfessional = async (req, res) => {
  ...
  if (professionalExists) {
    return res
      .status(400)
      .json({ message: "Profissional com esse email já existe!" });
  }
  ...
  try {
    ...
    return res
      .status(201)
      .json({ message: "Profissional criado com sucesso!", professional });
  } catch (error) {
    return res
      .status(400)
      .json({ message: "Erro ao criar o profissional", error: error.message });
  }
};
...
const listProfessionals = async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({
      message: "Erro ao listar os profissionais",
      error: error.message,
    });
  }
};
...
const listProfessionalById = async (req, res) => {
  ...
  if (!professionalId) {
    return res
      .status(400)
      .json({ message: "ID do profissional não fornecido!" });
  }
...
const assignRole = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Role atribuída com sucesso!", professional });
  } catch (error) {
    return res
      .status(400)
      .json({ message: "Erro ao atribuir a role", error: error.message });
  }
};
